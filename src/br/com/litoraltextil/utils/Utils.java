...
public class Utils {
    public static void AlteraProdutoPedido(DynamicVO deParaVO) throws Exception {
...
//            DynamicVO produtoAntVO = ProdutoDAO.get(codprodant);
//            DynamicVO pantoneAntVO = ProdutoDAO.getPantone(produtoAntVO.asBigDecimal("AD_CODCOR"));
//            System.out.println("teste do caralho" + pantoneAntVO.asString("CODHEX"));
            BigDecimal codprodins = deParaVO.asBigDecimal("CODPRODINS");
//            DynamicVO produtoInsVO = ProdutoDAO.get(codprodins);
//            DynamicVO pantoneInsVO = ProdutoDAO.getPantone(produtoInsVO.asBigDecimal("AD_CODCOR"));
//            System.out.println("teste do caralho" + pantoneAntVO.asString("CODHEX"));
            BigDecimal codproj = deParaVO.asBigDecimal("CODPROJ");
            String corAnterior = deParaVO.asString("CODCORANT");
            String corInserida = deParaVO.asString("CODCORINS");
            String nomeProduto = deParaVO.asString("PRODUTOANT");
            Timestamp dtAlter = deParaVO.asTimestamp("DTALTER");
            System.out.println("Pedido: xxxxxxx");

            NativeSql sql = new NativeSql(jdbc);
            sql.appendSql("SELECT ITE.NUNOTA , ITE.SEQUENCIA , ITE.QTDNEG FROM TGFITE ITE WHERE CODPROD = :CODPROD AND CONTROLE = :CONTROLE AND CODLOCALORIG = 1002 AND NOT EXISTS (SELECT 1 FROM TGFVAR WHERE NUNOTAORIG = ITE.NUNOTA)");
            sql.setNamedParameter("CODPROD", codprodant);
            sql.setNamedParameter("CONTROLE", codproj.toString());
            ResultSet rs = sql.executeQuery();
            System.out.println("Pedido: xxxxxxx2");
            while (rs.next()) {
                BigDecimal nunota = rs.getBigDecimal("NUNOTA");
                BigDecimal sequencia = rs.getBigDecimal("SEQUENCIA");
                Double qtdNeg = rs.getDouble("QTDNEG");
...
//                        .set("CODHEXANT", pantoneAntVO.asString("CODHEX"))
                        .set("CODPRODINS", codprodins)
//                        .set("CODHEXINS", pantoneInsVO.asString("CODHEX"))
                        .set("CODPROJ", codproj.toString())
                        .set("SEQITE", sequencia)
                        .set("DTALTER", dtAlter)
                        .set("CODUSU", codUsu)
                        .set("CODCORANT", corAnterior)
                        .set("CODCORINS", corInserida)
                        .set("PRODUTOANT", nomeProduto)
                        .set("QTDNEG", qtdNeg)
                        .save();
            }
        } catch (Exception e) {
...
//    public static void NotificarVendedor(BigDecimal nunota , List<BigDecimal> codProdAnt , List<BigDecimal> codProdIns ) throws Exception {
//        try {
//
...
//    }
    ...
    public static String validarOperacao( BigDecimal nunota, BigDecimal codProdAtual, BigDecimal codProdNovo, BigDecimal codProj) throws Exception {
        ...
        DynamicVO itemNotaVO = ItemNotaDAO.getIteValid(nunota, codProdNovo);
        ...
        DynamicVO familiaProdAtualVO = TabelasAD.familiaProdDAO(codProdAtual);
        DynamicVO familiaProdNovoVO = TabelasAD.familiaProdDAO(codProdNovo);
        if (familiaProdNovoVO.asBigDecimal("CODPRODGR2").compareTo(familiaProdAtualVO.asBigDecimal("CODPRODGR2")) != 0) {
            return "Não é possivel alterar o item para um produto de um \"PAI\" diferente do projeto.<br>";
        }
        ...
        DynamicVO deParaVO = TabelasAD.deParaProdDAO(codProdNovo);
        if (deParaVO != null) {
    ...
    public static void inserirDePara(BigDecimal codProj, BigDecimal codProdAnterior, BigDecimal codProdNovo, String corAnterior, String corNova,String produtoAnterior, String produtoNovo, String nomeUsuario, BigDecimal nunota, BigDecimal sequencia, String notifica) throws Exception {
        try {
            JapeWrapper deParaDAO = JapeFactory.dao("AD_DEPARA");
            deParaDAO.create()
                    .set("ID", NumeracaoDAO.getDeParaID())
                    .set("CODDEPARA", NumeracaoDAO.getDeParaCOD())
                    .set("CODPROJ", codProj)
                    .set("CODPRODANT", codProdAnterior)
                    .set("CODPRODINS", codProdNovo)
                    .set("CODCORANT", corAnterior)
                    .set("CODCORINS", corNova)
                    .set("USUARIO", nomeUsuario)
                    .set("DTALTER", new Timestamp(System.currentTimeMillis()))
                    .set("PRODUTOANT", produtoAnterior)
                    .set("PRODUTOINS", produtoNovo)
                    .set("NUNOTA", nunota)
            ...
            e.printStackTrace();
            throw new Exception("Erro ao inserir registro na tabela AD_DEPARA: " + e.getMessage());
        }
    }

    public static void enviarEmailDePara(StringBuilder email , String usuario , BigDecimal codproj ) throws Exception {
        JdbcWrapper jdbc = EntityFacadeFactory.getDWFFacade().getJdbcWrapper();
        try {
            StringBuilder emailFinal = new StringBuilder();
            emailFinal.append(email);
            emailFinal.append("<tr style=\"text-align: center;\">");
            emailFinal.append("<th>Produto Pai</th>");
            emailFinal.append("<th>Produto Substituido</th>");
            emailFinal.append("<th>Produto Substituto</th>");
            emailFinal.append("<th>Cor Substituida</th>");
            emailFinal.append("<th>Cor Substituta</th>");
            emailFinal.append("<th>Status</th>");
            emailFinal.append("</tr>");
            emailFinal.append("</table><p>Itens alterados por: ").append(usuario).append(" em ").append(new SimpleDateFormat("dd/MM/yyyy HH:mm").format(new Timestamp(System.currentTimeMillis()))).append(".</p></html>");

            JapeWrapper tmdfmgDAO = JapeFactory.dao(DynamicEntityNames.FILA_MSG);
            tmdfmgDAO.create()
//                    .set("EMAIL", "depara@litoraltextil.com.br")
                    .set("EMAIL", "francisco.silva@litoraltextil.com.br")
                    .set("CODCON", BigDecimal.ZERO)
                    .set("CODMSG", null)
                    .set("STATUS", "Pendente")
                    .set("TIPOENVIO", "E")
                    .set("MAXTENTENVIO", BigDecimal.valueOf(3))
                    .set("ASSUNTO", "AJUSTE DE COR - Projeto: " + codproj)
                    .set("MENSAGEM", emailFinal.toString().toCharArray())
                    .set("CODSMTP", BigDecimal.TEN)
                    .save();

            NativeSql sql = new NativeSql(jdbc);
            sql.appendSql("DELETE AD_ORDDEPARA");
            sql.executeUpdate();
